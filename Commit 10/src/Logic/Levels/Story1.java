...
public class Story1 implements ICrossingStrategy{
	...
	public boolean isValid(List<ICrosser> rightBankCrossers, List<ICrosser> leftBankCrossers,
		...
		//win (not sure)
		if(leftBankCrossers.size()==0)
			return true;
		
	    //check first if there are any boat riders
		...
		if(errorNumber==1) {
			...
			receiver.setErrorMessage(errorMessage);
			return false;
		}
		...
		if(rightBankCrossers.size()!=4) {
			...
			for(int i=0;i<rightBankCrossers.size()-1;i++) {
				if(rightBankCrossers.get(i) instanceof Farmer ||rightBankCrossers.get(i+1) instanceof Farmer) {
					errorNumber=0;
					break;
				}		
		...
		if(errorNumber==2) {
			if((rightBankCrossers.get(0) instanceof Wolf && rightBankCrossers.get(rightBankCrossers.size()-1) instanceof Goat) || 
					(rightBankCrossers.get(0) instanceof Goat && rightBankCrossers.get(rightBankCrossers.size()-1) instanceof Wolf))
				errorMessage="Wolf will eat the goat.";
		...
		if(leftBankCrossers.size()!=4) {
			...
			for(int i=0;i<leftBankCrossers.size()-1;i++) {
				if(leftBankCrossers.get(i) instanceof Farmer || leftBankCrossers.get(i+1) instanceof Farmer ) {
					errorNumber=0;
					break;
				}	
		...
		if(errorNumber==3) {
			if((leftBankCrossers.get(0) instanceof Wolf && leftBankCrossers.get(leftBankCrossers.size()-1) instanceof Goat) || 
					(leftBankCrossers.get(0) instanceof Goat && leftBankCrossers.get(leftBankCrossers.size()-1) instanceof Wolf))
				errorMessage="Wolf will eat the goat.";
